...
use CKPL\Pay\Model\Request\SelectedPaymentMethodRequestModel;
...
class MakePaymentEndpoint implements EndpointInterface
{
     ...
     * @type string
     */
    const PARAMETER_STORE_CUSTOMER = 'store_customer';

    /**
     * @type string
     */
    const PARAMETER_SELECTED_PAYMENT_METHOD = 'selected_payment_method';
     ...
     * @type bool
     */
    const PARAMETER_RETRY_ENABLED = 'retry_enabled';
    ...
    public function processRawInput(array $parameters): ?ProcessedInputInterface
    {
        ...
        if (isset($parameters[static::PARAMETER_STORE_CUSTOMER])) {
            $storeCustomer = $parameters[static::PARAMETER_STORE_CUSTOMER];
        ...
        if (isset($parameters[static::PARAMETER_SELECTED_PAYMENT_METHOD])) {
            $selectedPaymentMethod = $parameters[static::PARAMETER_SELECTED_PAYMENT_METHOD];
            $selectedPaymentMethodModel = new SelectedPaymentMethodRequestModel();
            $selectedPaymentMethodModel->setType($selectedPaymentMethod->getType());
            $selectedPaymentMethodModel->setIssuer($selectedPaymentMethod->getIssuer());
            $model->setSelectedPaymentMethod($selectedPaymentMethodModel);
        }
        ...
        if (isset($parameters[static::PARAMETER_RETRY_ENABLED])) {
            $model->setRetryEnabled($parameters[static::PARAMETER_RETRY_ENABLED]);
        }
