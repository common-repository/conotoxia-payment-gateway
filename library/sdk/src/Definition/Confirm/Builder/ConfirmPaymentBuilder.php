...
class ConfirmPaymentBuilder implements ConfirmPaymentBuilderInterface
{
     ...
     * @return $this
     */
    public function setAliasName(string $aliasName): ConfirmPaymentBuilder
    {
        $this->confirmPayment->setAliasName($aliasName);
     ...
     * @return $this
     */
    public function setAliasKey(string $aliasKey): ConfirmPaymentBuilder
    {
        $this->confirmPayment->setAliasKey($aliasKey);
     ...
     * @return $this
     */
    public function setCustomerId(string $customerId): ConfirmPaymentBuilder
    {
        $this->confirmPayment->setCustomerId($customerId);
    ...
    public function getConfirmPayment(): ConfirmPaymentInterface
    {
        ...
        if (null === $this->confirmPayment->getBlikCode()) {
            if (null === $this->confirmPayment->getAliasName()) {
                throw new ConfirmPaymentException('Missing alias name in confirm payment request.');
            }

            if (null === $this->confirmPayment->getAliasKey()) {
                throw new ConfirmPaymentException('Missing alias key in confirm payment request.');
            }

            if (null === $this->confirmPayment->getCustomerId()) {
                throw new ConfirmPaymentException('Missing customer id in confirm payment request.');
            }
        }  else {
            if (empty($this->confirmPayment->getBlikCode())) {
                throw new ConfirmPaymentException('Blik code is empty in confirm payment request.');
            }
        }
